...
void DDialog::insertContent(int index, QWidget *widget, Qt::Alignment alignment)
{
    ...
    // 在这里做防御性编程：如果插入的index越界了直接给定为当前局部的count
    // So here is my defense strategy here: if index out of bound/range, force set it
    // to the current index count.
    int targetIndex = index + DIALOG::CONTENT_INSERT_OFFSET;
    if (targetIndex > d->contentLayout->count()) {
        targetIndex = d->contentLayout->count();
    }

    d->contentLayout->insertWidget(targetIndex, widget, 0, alignment);
